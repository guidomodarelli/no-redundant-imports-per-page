...
describe('no-redundant-imports-per-page rule', () => {
  it('reports direct duplicate imports inside a page entry', async () => {
    ...
    expect(messages[1]?.message).toBe(
      `Duplicated import "@root/app/components/SearchDropdown/styles" in "${getFixturePath('app/pages/userCreate/styles.scss')}". First seen at "${getFixturePath('app/pages/userCreate/styles.scss')}:2" for page "${getFixturePath('app/pages/userCreate/styles.scss')}".`,
    );
  ...
  it('supports multiple imports declared in the same statement', async () => {
    ...
    expect(messages[0]?.line).toBe(1);
    expect(messages[0]?.message).toBe(
      `Duplicated import "@root/app/components/SearchDropdown/styles" in "${getFixturePath('app/pages/multiImport/styles.scss')}". First seen at "${getFixturePath('app/pages/multiImport/styles.scss')}:1" for page "${getFixturePath('app/pages/multiImport/styles.scss')}".`,
    );
  });
  ...
  it('reports cycles as redundant imports', async () => {
    ...
    expect(messages[0]?.line).toBe(1);
    expect(messages[0]?.message).toBe(
      `Duplicated import "~@root/app/components/CycleA/styles" in "${getFixturePath('app/components/CycleB/styles.scss')}". First seen at "${getFixturePath('app/pages/cycle/styles.scss')}:1" for page "${getFixturePath('app/pages/cycle/styles.scss')}". Cycle detected in the dependency chain.`,
    );
  });
  ...
  it('detects one-level internal node_modules duplicates in simple mode with depth 1', async () => {
    ...
    expect(messages[0]?.line).toBe(1);
    expect(messages[0]?.message).toBe(
      `Duplicated import "~@test/one-level/shared" in "${getFixturePath('app/components/OneLevelSharedConsumer/styles.scss')}". First seen at "${getFixturePath('node_modules/@test/one-level/index.scss')}:1" for page "${getFixturePath('app/pages/nodeModulesOneLevel/styles.scss')}".`,
    );
  });
  ...
  it('detects deeper node_modules duplicates in advanced mode', async () => {
    ...
    expect(messages[0]?.line).toBe(1);
    expect(messages[0]?.message).toBe(
      `Duplicated import "~@test/deep-chain/shared" in "${getFixturePath('app/components/DeepChainSharedConsumer/styles.scss')}". First seen at "${getFixturePath('node_modules/@test/deep-chain/layer.scss')}:1" for page "${getFixturePath('app/pages/nodeModulesDeepChain/styles.scss')}".`,
    );
  });
  ...
  it('reads aliases from package.json _moduleAliases', async () => {
    ...
    expect(messages[0]?.line).toBe(2);
    expect(messages[0]?.message).toBe(
      `Duplicated import "@app/styles/common" in "${getFixturePath('app/pages/packageAliases/styles.scss')}". First seen at "${getFixturePath('app/pages/packageAliases/styles.scss')}:1" for page "${getFixturePath('app/pages/packageAliases/styles.scss')}".`,
    );
  });
